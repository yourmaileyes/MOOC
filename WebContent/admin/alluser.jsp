				...
				<td colspan="5" style="font-size: 35px;text-align:center;">暂无订单 </td>
				 ...
				 <c:forEach items = "${users }" var = "order">
					...
					<tr>
						...
						<a class="btn btn-danger delete" style="background-color: gray;" href="banuser.do?userid=${order.id }&type=1">恢复登录</a>
						</c:if>
						<c:if test="${order.buycase!=1}">
						<a class="btn btn-danger delete"  href="banuser.do?userid=${order.id }&type=0">禁止登录</a>
						</c:if>
						</c:otherwise>
						</c:choose>
						 </td>
						...
						<c:otherwise>
							<a class="btn btn-danger delete" style="background-color: green;" href="newuser.do?userid=${order.id }">查看详情</a>
						 </c:otherwise>
						 </c:choose>
						 </td>
						 <td class="col-md-2 text-center ">
