				...
				<td colspan="5" style="font-size: 35px;text-align:center;">暂无IP </td>
				 ...
				 <c:forEach  items = "${ips}" var = "order" >
					<tr>
						...
						<td class="col-md-2 text-center">
						<a class="btn btn-danger delete" style="background-color: red;" href="ipset.do?ip=${order.ip }">封禁IP</a>
						</td>
						<td class="col-md-2 text-center">
						<c:if test="${order.type eq '1'}">
						<a class="btn btn-danger delete" style="background-color: red;" href="ipset.do?ip=${order.ip }&onbaned=onbaned">解除封禁</a>
						</c:if>
						</td>
					</tr>
